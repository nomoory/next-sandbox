...
const NewsDetail = ({ id, newsStore }) => {
  useEffect(() => {
    newsStore.loadById(id);
    return () => {
      newsStore.initData();
    };
  }, []);
  const { title, content, updatedAt, createdAt } = newsStore.data || {};
...
export default inject("newsStore")(observer(NewsDetail));
